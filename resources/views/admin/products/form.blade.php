...
@section('title', $product ? __('global.edit_product') : __('global.new_product'))

@section('content')
<a href="{{ route('admin.products.index') }}" class="pm-link d-inline-flex align-items-center mb-2">&larr; {{ __('global.go_back') }}</a>
  ...
  <div class="pm-card-body">
    ...
    <form class="pm-form" method="POST" action="{{ $product ? route('admin.products.update',$product) : route('admin.products.store') }}">
      ...
      <div class="mt-3 d-flex gap-2">
        <button class="btn pm-btn pm-btn-primary">{{ $product ? 'Salvar' : __('global.create') }}</button>
        <a href="{{ route('admin.products.index') }}" class="btn pm-btn pm-btn-outline-secondary">{{ __('global.cancel') }}</a>
      </div>
    </form>
  </div>
</div>
